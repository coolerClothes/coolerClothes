...
onMounted(() => {
    ...
    let deliveryInfoInputs = Array.from(deliveryInfo.value.getElementsByTagName('input'));
    ...
    /* Check if paymentMethod fieldset is valid */
    ...
    <div class="2xl:px-[20%] px-4 my-4 overflow-x-hidden">
        ...
        <form class="font-inter font-light text-[#F5F5F5]">
            ...
            <div class="grid p-4 grid-cols-5 grid-rows-20 gap-4 2xl:gap-[1vw] font-inter ">
                ...
                <div class="bg-[#1c1c1c] col-span-3 row-span-3 px-6 pt-8 pb-10 border-solid border-l-4 border-[#00e0ff]"
                    :class="{ 'border-solid border-l-4 border-[#4dc94f]': !shipFieldsetDisabled }">
                    <fieldset id="deliveryInfo" ref="deliveryInfo" class="text-[#0c0c0c]">
                        ...
                        <div class="relative inline-flex w-[48.5%]">
                            <input id="email" aria-label="Email" type="email" placeholder="Email" required
                                maxlength="40" class="w-full">
                            <p
                                class="tooltip absolute bottom-[100%] px-1 bg-[#00E0FF] rounded-t-sm self-center text-[#003238] invisible text-xs font-medium z-50 flex">
                                Email:
                                example@mail.example <span class="check invisible font-bold absolute right-1">✓</span>
                                <span class="cross invisible font-bold ml-auto">✗</span>
                            </p>
                        </div>
                        <div class="relative inline-flex w-[48.5%]">
                            <input id="phoneNumber" aria-label="Phone number" type="text" placeholder="Phone number"
                                class="w-full" pattern="(\+?(\d\s?-?){11}|(\d\s?-?){10})" required>
                            <p
                                class="tooltip absolute bottom-[100%] bg-[#00E0FF] px-1 rounded-t-sm self-center text-[#003238] invisible text-xs font-medium z-50">
                        ...
                        <div class="relative inline-flex w-[48.5%]">
                            <input id="firstName" aria-label="First name" type="text" placeholder="First name"
                                class="w-full" pattern="[\p{L}a-z]{1,30}" required>
                            <p
                                class="tooltip absolute bottom-[100%] bg-[#00E0FF] px-1 rounded-t-sm self-center text-[#003238] invisible text-xs font-medium z-50">
                        ...
                        <div class="relative inline-flex w-[35.5%]">
                            <input id="adress" aria-label="Delivery adress" type="text" placeholder="Delivery adress"
                                class="w-full" pattern="[\p{L}a-z]{0,30}\s?\d{1,3}" required>
                            <p
                                class="tooltip bg-[#00E0FF] absolute bottom-[100%] px-1 rounded-t-sm self-center text-[#003238] invisible text-xs font-medium z-50">
                        ...
                        <div class="relative inline-flex w-[30%]">
                            <input id="postCode" aria-label="Post code" type="text" placeholder="ZIP code" required
                                class="w-full" pattern="\d{3}\s?\d{2}">
                            <p
                                class="tooltip bg-[#00E0FF] absolute bottom-[100%] px-1 rounded-t-sm self-center text-[#003238] invisible text-xs font-medium z-50">
                        ...
                        <div class="relative inline-flex w-[30%]">
                            <input id="postTown" aria-label="Post town" type="text" placeholder="City" required
                                class="w-full" pattern="[\p{L}a-z]{1,15}">
                            <p
                                class="tooltip bg-[#00E0FF] absolute bottom-[100%] px-1 rounded-t-sm self-center text-[#003238] invisible text-xs font-medium z-50">
                ...
                <!-- shipment method div -->
                <div class="bg-[#1c1c1c] col-span-3 row-span-3 px-6 pt-8 pb-10 border-solid border-l-4 border-[#00e0ff]"
                    :class="{ 'opacity-40 filter grayscale': shipFieldsetDisabled }, { 'border-solid border-l-4 border-[#4dc94f]': !payFieldsetDisabled }">
                ...
                <!-- payment method div -->
                <div class="bg-[#1c1c1c] col-span-3 row-span-4 px-6 pt-8 pb-10 border-solid border-l-4 border-[#00e0ff]"
                    :class="{ 'opacity-40 filter grayscale': payFieldsetDisabled }, { 'border-solid border-l-4 border-[#4dc94f]': !confirmFieldsetDisabled }">
                    ...
                    <router-link to="/orderConfirmation">
                        <button id="confirmButton" label="Confirm payment"
                            ...
                            <div id="button-contents" class="relative p-1.5">
                                ...
                                <svg id="added-icon" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
